...
{
	public:
		EDPComm(SoftwareSerial *serialPort, Akeru *board);
		...
		int getLoadProfileData(uint16_t dataArray[16]);
		...
		/*SigFox Data*/
		int getSGFXYearData(uint16_t dataArray[16]);
		int getSGFXMonthData(uint16_t dataArray[16]);
		void sendToSigfox(const String payload);
