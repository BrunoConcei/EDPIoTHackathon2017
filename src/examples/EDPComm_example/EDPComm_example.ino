...
//For the Akeru board TX=4 and RX=5
#define TX 4
#define RX 5
...
// Initialize Libs
Akeru akeru(RX, TX);                //initalize akeru board library
...
void loop()
{
  ...
  case 2:
    ...
    {
      u32wait = millis() + 2000;
      edpComm.printLoadProfileData(au16data); //prints loadProfileData from EB
      u32wait = millis() + 2000;
  ...
  case 6:
    /*
        ...
        */
    Serial.println("**Starting SIGFOX comm...**");
    ...
    if (edpComm.checkLoadProfileData(au16data))
    {
      ...
      String sData = akeru.toHex(loadProfileData);
      Serial.print(".sData:" + sData);
      String sgfxMsg = sData; // Put everything together (just concatenate strings)
      Serial.println(".sgfMsg:" + sgfxMsg + "|");
      edpComm.sendToSigfox(sgfxMsg); //use this function instead of Akeru default one
